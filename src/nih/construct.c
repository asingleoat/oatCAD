...
static int init_query_structure(int segnum, segment_t *seg, node_t *qs, trap_t *tr)
{
...
static int is_left_of(uint segnum, segment_t *seg, point_t *v)
{
  segment_t *s = &seg[segnum];
  double area;
...
static uint inserted(int segnum, segment_t *seg, node_t *qs, trap_t *tr, int whichpt)
{
  if (whichpt == FIRSTPT)
 ...
 */
/* segment_t *seg, , trap_t *tr */
uint locate_endpoint(point_t *v, point_t *vo, uint r, segment_t *seg, node_t *qs)
{
  node_t *rptr = &qs[r];
...
static int merge_trapezoids(int segnum, segment_t *seg, node_t *qs, trap_t *tr, int tfirst, int tlast, int side)
{
  int t, tnext, cond;
  int ptnext;
...
static int add_segment(uint segnum, segment_t *seg, node_t *qs, trap_t *tr)
{
  segment_t s;
  ...
  if ((is_swapped) ? !inserted(segnum, seg, qs, tr, LASTPT) :
       !inserted(segnum, seg, qs, tr, FIRSTPT))     /* insert v0 in the tree */
    {
      int tmp_d;
  ...
  if ((is_swapped) ? !inserted(segnum, seg, qs, tr, FIRSTPT) :
       !inserted(segnum, seg, qs, tr, LASTPT))     /* insert v1 in the tree */
    {
      int tmp_d;
  ...
  tfirstl = tfirst;
  tlastl = tlast;
  merge_trapezoids(segnum, seg, qs, tr, tfirstl, tlastl, S_LEFT);
  merge_trapezoids(segnum, seg, qs, tr, tfirstr, tlastr, S_RIGHT);
 ...
 */
static int find_new_roots(uint segnum, segment_t *seg, node_t *qs, trap_t *tr)
{
  segment_t *s = &seg[segnum];
...
uint construct_trapezoids(uint nseg, segment_t *seg, node_t *qs, trap_t *tr)
{
  uint i;
  uint root, h;
  ...
  /* list initialised */
  root = init_query_structure(choose_segment(), seg, qs, tr);
  ...
  for (h = 1; h <= math_logstar_n(nseg); h++)
	...
	add_segment(choose_segment(), seg, qs, tr);
      
      /* Find a new root for each of the segment endpoints */
      for (i = 1; i <= nseg; i++)
        find_new_roots(i, seg, qs, tr);
    }
