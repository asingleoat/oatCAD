...
uint read_segments(char *filename, uint *genus, segment_t *seg)
{
  FILE *infile;
  uint ccount;
  uint i;
  uint ncontours, npoints, first, last;
...
int int_log2(int x) {
    if (x == 0) {
        fprintf(stderr, "Error: log2(0) is undefined.\n");
        return 0; // Handle invalid input safely
    }

    int result = 0;

    // Shift until the number becomes 0
    while (x > 1) {
        x >>= 1;
        result++;
    }
...
int div_ceil(int a, int b) {
    if (b > 0) {
        ...
        fprintf(stderr, "Error: div by (0) is undefined.\n");
        return 0;
    }
}
