...
extern int monotonate_trapezoids(uint, segment_t *, trap_t *);
extern int triangulate_monotone_polygons(uint, uint, uint (*[3]));
...
extern uint locate_endpoint(point_t *, point_t *, uint, segment_t *, node_t *);
extern uint construct_trapezoids(uint, segment_t *, node_t *, trap_t *);

extern int generate_random_ordering(int);
extern int choose_segment(void);
extern uint read_segments(char *, uint *, segment_t *);
