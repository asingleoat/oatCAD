...
static uint mon[SEGSIZE];	/* contains position of any vertex in */
				/* the monotone chain for the polygon */
static uint visited[TRSIZE];
...
int monotonate_trapezoids(uint n, segment_t *seg, trap_t *tr)
{
  uint i;
  uint tr_start;
  ...
  if (tr[tr_start].u0 > 0)
    traverse_polygon(0, tr_start, tr[tr_start].u0, TR_FROM_UP, seg, tr);
  else if (tr[tr_start].d0 > 0)
    traverse_polygon(0, tr_start, tr[tr_start].d0, TR_FROM_DN, seg, tr);
...
static uint traverse_polygon(uint mcur, uint trnum, uint from, uint dir, segment_t *seg, trap_t *tr)
{
  trap_t *t = &tr[trnum];
  int howsplit, mnew;
	  ...
	  if (from == t->d1)
	    {
	      ...
	      traverse_polygon(mcur, t->d1, trnum, TR_FROM_UP, seg, tr);
	      traverse_polygon(mnew, t->d0, trnum, TR_FROM_UP, seg, tr);
	    }
	  else
	    {
	      mnew = make_new_monotone_poly(mcur, v0, v1);
	      traverse_polygon(mcur, t->d0, trnum, TR_FROM_UP, seg, tr);
	      traverse_polygon(mnew, t->d1, trnum, TR_FROM_UP, seg, tr);
	    }
	}
      else
	{
	  retval = SP_NOSPLIT;	/* Just traverse all neighbours */
	  traverse_polygon(mcur, t->u0, trnum, TR_FROM_DN, seg, tr);
	  traverse_polygon(mcur, t->u1, trnum, TR_FROM_DN, seg, tr);
	  traverse_polygon(mcur, t->d0, trnum, TR_FROM_UP, seg, tr);
	  traverse_polygon(mcur, t->d1, trnum, TR_FROM_UP, seg, tr);
	}
    }
	  ...
	  if (from == t->u1)
	    {
	      ...
	      mnew = make_new_monotone_poly(mcur, v1, v0);
	      traverse_polygon(mcur, t->u1, trnum, TR_FROM_DN, seg, tr);
	      traverse_polygon(mnew, t->u0, trnum, TR_FROM_DN, seg, tr);
	    }
	  else
	    {
	      mnew = make_new_monotone_poly(mcur, v0, v1);
	      traverse_polygon(mcur, t->u0, trnum, TR_FROM_DN, seg, tr);
	      traverse_polygon(mnew, t->u1, trnum, TR_FROM_DN, seg, tr);
	    }
	}
      else
	{
...
int triangulate_monotone_polygons(uint nvert, uint nmonpoly, uint* op[3]) {
  ...
  point_t ymax, ymin;
  uint p;
  uint vfirst, posmax, posmin, v;
  uint vcount, processed;
...
{
  uint v;
  uint rc[SEGSIZE], ri = 0;	/* reflex chain */
  uint endv, tmp, vpos;
